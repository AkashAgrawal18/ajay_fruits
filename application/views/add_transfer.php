...
<section class="py-3 d-flex" style="background:#f3f3ff;min-height:75vh;">
    <div class="container-fluid">
        ...
        <form method="post" action="#" id="frm-add-transfer">
            ...
            <div class="row mb-1 g-3">
                    ...
                    <div class="row">
                        ...
                        <div class="col-9">
                            <div class="form-group">
                                <input type="date" name="transfer_date" id="transfer_date" class="form-control" required value="<?= date('Y-m-d') ?>" max="<?= date('Y-m-d') ?>">
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <div class="col-md-4">
                    <div class="row">
                        <div class="col-4">
                            <label>Transfer To Branch <span class="text-danger">*</span></label>
                        </div>
                        <div class="col-8">
                            <div class="form-group">
                                <select name="to_branch" id="to_branch" class="form-select select2" required autofocus>
                ...
                <div class="col-md-12 mb-2">
                    ...
                    <div class="table-responsive">
                        ...
                        <input list="stock_datalist" id="item_serch_inp" placeholder="Search available Head Office stock by item/lot" class="form-control" style="width: 30%;">

                        <datalist id="stock_datalist">
                            <?php
                            if (!empty($stock_list)) {
                                foreach ($stock_list as $li) {
                                    $label = $li->m_item_name . ' - Lot ' . $li->m_purcs_lot . ' (Avail: ' . $li->m_purcs_available . ')';
                            ?>
                                    <option value="<?= $label ?>"
                                        ...
                                        data-cost="<?= $li->m_purcs_price ?>"
                                        data-landedrate="<?= $li->landed_rate ?>"><?= $label ?></option>
                            <?php
                                }
                            }
                            ?>
                ...
                <div class="col-md-2 text-end fw-bold">
                    ...
                    <div class="form-layout-submit text-end mt-2">
                        <button type="submit" id="btn-add-transfer" class="btn btn-block btn-warning">Create Transfer</button>
...
<script>
    $(document).ready(function(e) {
        ...
        $(document).on('change', '#item_serch_inp', function() {
            var $opt = $("#stock_datalist option[value='" + $(this).val() + "']");
            if (!$opt.length) {
                $(this).val('');
                return;
            }
            ...
            var landedrate = $opt.attr('data-landedrate');

            incount++;
            addrow(incount, lotid, itemname, lot, available, cost, landedrate);
        ...
        $('#frm-add-transfer').on('submit', function(ev) {
            ev.preventDefault();

            if (!$('#to_branch').val()) {
                swal('Please select a branch to transfer to.', {
                    icon: 'warning'
                });
                return;
            }
            ...
            if ($('.prodqty').length === 0) {
                swal('Please add at least one item to transfer.', {
                    icon: 'warning'
                });
                return;
            }
            ...
            if (invalid) {
                swal('Please enter a valid Transfer Qty (within available stock) and Issue Rate for every item.', {
                    icon: 'error'
                });
                return;
            }

            var $btn = $('#btn-add-transfer');
            $btn.prop('disabled', true).text('Saving...');

            $.ajax({
                url: "<?= base_url('Transfer/insert_transfer') ?>",
                type: "POST",
                data: $(this).serialize(),
                dataType: 'json',
                success: function(res) {
                    $btn.prop('disabled', false).text('Create Transfer');
    ...
    function addrow(x, lotid, itemname, lot, available, cost, landedrate) {
        $('#tableblock').append(`<tr id="rowcot` + x + `">
            ...
            <td>` + available + `</td>
            <td>` + cost + `</td>
            <td><input type="number" min="0.01" step="0.01" max="` + available + `" id="qty` + x + `" name="qty[]" class="prodqty calcuclass" data-count="` + x + `" data-max="` + available + `" style="width:90px" value="` + available + `"></td>
            <td><input type="number" min="0.01" step="0.01" id="rate` + x + `" name="rate[]" class="prodrate calcuclass" data-count="` + x + `" style="width:90px" value="` + landedrate + `"></td>
